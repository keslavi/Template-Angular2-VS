...
This is a minor fork of mgechev's Angular-Seed with the following additions:

- Bootstrap/ng2-bootstrap
- Font support for Bootstrap
- JQuery (for the modal dialogs)
- angular2-toaster for popups
- image support in /assets/img 
- a template component communicating with the VS /Server backend (/server/api/controller)
- a sample vs WebAPI configured to run /client/dist/dev at root, WebApi at /server/api/*

Setting up the VS applicationhost.config file:
the applicationhost.config file controls IISExpress directory settings in a fashion similar to an Apache Hosts file. The settings shown here will work for VS prior to 2015, but the config file is in a different location. as of 2015 Microsoft makes the config file together with the *.sln files under /.vs/config/applicationhost.config.

IIS Express Layout: 
  /                     -root, pointing to /client/dist/dev
    /server             -WebApi project
      /api/{controller} -backend controllers
...
```bash
$ git clone --depth 1 https://github.com/keslavi/Template-Angular2-VS.git
$ cd Template-Angular2-VS
